...
function normalizeSessionFilter(value: string | undefined): SessionFilter {
  ...
  if (value === "has-responses") return "has-responses";
  if (value === "no-responses") return "no-responses";
  return "all";
}
...
function filterSessions(
  ...
  if (filter === "closed") return sessions.filter((s) => !s.is_active);
  if (filter === "has-responses")
    return sessions.filter((s) => s.responseCount > 0);
...
const filterOptions: { value: SessionFilter; label: string; href: string }[] = [
  { value: "all", label: "전체", href: "/teacher/sessions" },
  { value: "active", label: "진행 중", href: "/teacher/sessions?filter=active" },
  { value: "closed", label: "종료", href: "/teacher/sessions?filter=closed" },
  {
    value: "has-responses",
    label: "응답 있음",
    href: "/teacher/sessions?filter=has-responses",
  },
  {
    value: "no-responses",
    label: "응답 없음",
    href: "/teacher/sessions?filter=no-responses",
  },
];

export default async function TeacherSessionsPage({
  searchParams,
}: TeacherSessionsProps) {
  ...
  return (
    <>
      <div className="mb-6">
        <p className={`text-sm font-semibold ${theme.accentText}`}>세션 관리</p>
        <h1 className="mt-1 text-2xl font-bold sm:text-3xl">수업 세션 만들기·진행 관리</h1>
        <p className="mt-1 text-sm text-slate-400">
          활동 세션을 만들고 입장 코드를 발급한 뒤, 학생 응답을 확인하고 종료
          처리합니다.
        </p>
      </div>
      ...
      {activitiesError ? (
        <div className="mb-4 rounded-2xl border border-red-300/30 bg-red-950/30 p-4 text-sm text-red-200">
          활동 데이터를 불러오지 못했습니다: {activitiesError.message}
        </div>
      ) : null}

      {responseCountError ? (
        <div className="mb-4 rounded-2xl border border-yellow-300/30 bg-yellow-950/30 p-4 text-sm text-yellow-100">
          응답 수 정보를 불러오지 못했습니다: {responseCountError.message}
        </div>
      ) : null}
      ...
      {activityList.length > 0 ? (
        ...
        <div className="mb-6 rounded-2xl border border-yellow-300/30 bg-yellow-950/30 p-6 text-yellow-100">
          활동 데이터가 없어서 수업 세션을 만들 수 없습니다.
        </div>
      )}

      <section className="mt-6 rounded-2xl border border-white/10 bg-slate-900/40 p-5 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h2 className="text-xl font-bold">최근 수업 세션</h2>
            <p className="mt-2 text-sm leading-6 text-slate-300">
              생성된 세션의 입장 코드와 학생 응답 수를 확인하고, 수업이 끝난
              세션을 종료하거나 삭제할 수 있습니다.
            </p>
          </div>
