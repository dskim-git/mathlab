    ...
    <div className="rounded-2xl border border-white/10 bg-slate-900 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="text-sm text-slate-400">
          ...
          <span className="font-bold text-cyan-300">{generalPerms.length}</span>
        </span>
      </div>
...
export default function AdminSubjectsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [subjectRes, classRes, generalRes, classPermRes, generalPermRes] =
      await Promise.all([
        supabase
          .from("subjects")
    ...
    <main className="min-h-screen px-6 py-10">
      <Card className="mx-auto max-w-5xl p-6 sm:p-8">
        ...
        <p className="mt-4 leading-7 text-slate-300">
          교과별로 학생(학급 단위)과 일반인(개인 단위)의 접근 권한을 부여·회수합니다.
          교사의 담당 과목·학급은{" "}
